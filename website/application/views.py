...
def login():
    ...
    if request.method == "POST":
        ...
        if len(name) >= 2:
            ...
            flash(f'You were successfullt logged in as {name}.')
...
def logut():
    """ 
    logs the user out by popping name for session
    """
...
def history():
    ...
    json_message = get_history(session[NAME_KEY])
    print(json_message)
    return render_template("history.html", **{"history": json_message})
